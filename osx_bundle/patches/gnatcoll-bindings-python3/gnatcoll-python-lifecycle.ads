...
package GNATCOLL.Python.Lifecycle is
   ...
   procedure Py_SetExecutable (Executable : String);
   --  Stash an absolute path to the embedding executable so that PyConfig
   --  .executable is set to it at Py_InitializeFromConfig time.  This keeps
   --  sys.executable pointing at the host binary (e.g. gnatstudio_exe),
   --  rather than at Python's compile-time default or a nearby /usr/bin/
   --  interpreter discovered by argv[0] walking.
   --  Safe to call at any point before Py_Initialize / the first
   --  Register_Python_Scripting.  An empty string clears the stash.
